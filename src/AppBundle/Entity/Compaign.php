 ...
 * @ORM\Entity(repositoryClass="AppBundle\Repository\CompaignRepository")
 */
class Compaign
{
    /**
    ...
    */
    public function getCompaigns ($doctrine, $date) 
    {
        ...
        $return['compaign'] = $em->getRepository('AppBundle:Compaign')->findAllWithType( $date );
        $return['typecompaign'] = $em->getRepository('AppBundle:typeCompaign')->findAll();
        $return['users'] = $em->getRepository('AppBundle:Users')->findBy( array('type' => 'membre') );

        // resultat ho any semaine ra anio no resahina
        // resulat ho any volana ra volana no resahina
        $return['marketing'] = $em->getRepository('AppBundle:Compaign')->findMarketing($date);
